...
validate_mapping_columns <- function(df, required_cols = c("marker", "CHROM", "POS", "log10p")) {
...
#' Validate that log10p values are reasonable
#'
#' @param df Mapping dataframe with log10p column
#' @return TRUE if valid, warning if issues detected
validate_log10p_values <- function(df) {
  if (any(is.na(df$log10p))) {
    n_na <- sum(is.na(df$log10p))
    warning(glue::glue("Found {n_na} NA values in log10p column"))
  }

  if (any(df$log10p < 0, na.rm = TRUE)) {
    n_neg <- sum(df$log10p < 0, na.rm = TRUE)
    warning(glue::glue("Found {n_neg} negative log10p values (should be positive)"))
  }
