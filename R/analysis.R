...
#' calculate_threshold("BF", n_markers = 13000)
#'
#' # EIGEN-based threshold
#' calculate_threshold("EIGEN", n_independent = 484.839)
#'
#' # User-specified threshold
#' calculate_threshold(5.5)
...
#'
#' @param df Mapping dataframe with log10p column
...
#' @return Dataframe with added columns: sig_threshold_value, sig_threshold_method, significant
...
#'
#' @param df Mapping dataframe from database query (must have CHROM, POS, log10p)
#' @param threshold_value Numeric threshold on -log10 scale
...
analyze_mapping <- function(df,
  ...
  # Step 1: Flag significant markers
...
#' 1. Query mapping data from database (with markers joined)
#' 2. Get threshold parameters (n_markers, n_independent_tests)
#' 3. Calculate threshold value based on method
#' 4. Apply threshold and flag significant markers
#' 5. Group significant markers into QTL intervals
#' 6. Define confidence intervals
#' 7. Extract QTL regions and summary statistics
...
#' Process multiple mappings efficiently in batch
#'
#' Processes thousands of mappings with optimized database access and
#' optional parallel execution.
#'
#' @param mapping_ids Character vector of mapping IDs to process
#' @param method Threshold method: "BF", "EIGEN", or numeric value
#' @param base_dir Database root directory
#' @param alpha Significance level (default: 0.05)
#' @param ci_size Number of markers for CI boundary (default: 150)
#' @param snp_grouping Distance for grouping markers into same peak (default: 1000)
#' @param return_full_data If TRUE, return processed mapping data; if FALSE, only summaries
#' @param progress_interval How often to log progress (default: every 100 mappings)
#' @param n_cores Number of cores for parallel processing (default: 1 for sequential)
#' @param batch_size Number of mappings to bulk-query at once (default: 100)
#' @return Dataframe with results for all mappings
#'
#' @details
#' Optimizations for batch processing:
#' - Uses cached database connection (avoids repeated connection overhead)
#' - Bulk-queries mappings in batches for efficient I/O
#' - Pre-fetches all threshold parameters once
#' - Suppresses verbose logging during processing
#' - Optional parallel processing with n_cores > 1
#' - Returns only summary data by default to minimize memory usage
batch_analyze_mappings <- function(mapping_ids,
                                   method = "EIGEN",
                                   base_dir = "data/db",
                                   alpha = 0.05,
                                   ci_size = 150,
  ...
  n_mappings <- length(mapping_ids)
  log_msg(glue::glue("Starting batch analysis of {n_mappings} mappings"))
  log_msg(glue::glue("  Method: {method}, Alpha: {alpha}"))
  log_msg(glue::glue("  CI size: {ci_size}, SNP grouping: {snp_grouping}"))
  log_msg(glue::glue("  Batch size: {batch_size}, Cores: {n_cores}"))

  # Pre-fetch all threshold parameters
  log_msg("Pre-fetching threshold parameters...")
  all_threshold_params <- get_all_marker_set_metadata(base_dir)

  # Build threshold lookup for fast access
  threshold_lookup <- list()
  for (i in seq_len(nrow(all_threshold_params))) {
    key <- paste0(all_threshold_params$population[i], "_",
                  all_threshold_params$maf[i])
    threshold_lookup[[key]] <- list(
      n_markers = all_threshold_params$n_markers[i],
      n_independent_tests = all_threshold_params$n_independent_tests[i]
    )
  }

  # Pre-compute thresholds per marker set (avoid redundant calculations)
  threshold_values <- list()
  for (key in names(threshold_lookup)) {
    params <- threshold_lookup[[key]]
    if (is.numeric(method)) {
      threshold_values[[key]] <- list(
        value = method,
        method = "USER"
      )
    } else if (toupper(method) == "BF") {
      threshold_values[[key]] <- list(
        value = -log10(alpha / params$n_markers),
        method = "BF"
      )
    } else if (toupper(method) == "EIGEN") {
      if (!is.na(params$n_independent_tests) && params$n_independent_tests > 0) {
        threshold_values[[key]] <- list(
          value = -log10(alpha / params$n_independent_tests),
          method = "EIGEN"
        )
      } else {
        threshold_values[[key]] <- list(value = NA, method = "EIGEN")
      }
    }
  }

  start_time <- Sys.time()
  results <- vector("list", n_mappings)
  full_data <- if (return_full_data) vector("list", n_mappings) else NULL

  # Process in batches for efficient bulk queries
 n_batches <- ceiling(n_mappings / batch_size)
  log_msg(glue::glue("Processing {n_batches} batches..."))

  for (batch_idx in seq_len(n_batches)) {
    batch_start <- (batch_idx - 1) * batch_size + 1
    batch_end <- min(batch_idx * batch_size, n_mappings)
    batch_ids <- mapping_ids[batch_start:batch_end]

    # Progress logging
    if (batch_idx == 1 || batch_idx %% max(1, n_batches %/% 10) == 0) {
      elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
      mappings_done <- batch_start - 1
      rate <- if (elapsed > 0) mappings_done / elapsed else 0
      remaining <- if (rate > 0) (n_mappings - mappings_done) / rate else NA
      log_msg(glue::glue(
        "Batch {batch_idx}/{n_batches}: mappings {batch_start}-{batch_end} | ",
        "{round(rate, 1)} mappings/sec | ETA: {round(remaining, 0)}s"
      ))
    }

    # Bulk query all mappings in this batch
    bulk_data <- query_bulk_for_threshold_analysis(batch_ids, base_dir)
    ...
    # Split bulk data by mapping_id
    mapping_data_list <- split(bulk_data, bulk_data$mapping_id)

    # Process each mapping in this batch
    process_one <- function(idx) {
      mapping_id <- mapping_ids[idx]
      mapping_data <- mapping_data_list[[mapping_id]]
      ...
      tryCatch({
        population <- mapping_data$population[1]
        maf <- mapping_data$maf[1]
        key <- paste0(population, "_", maf)
        thresh <- threshold_values[[key]]

        if (is.null(thresh) || is.na(thresh$value)) {
          return(data.frame(
            mapping_id = mapping_id,
            status = "error",
            error_msg = "Threshold not available",
            stringsAsFactors = FALSE
          ))
        }

        # Run analysis with verbose=FALSE
        processed <- analyze_mapping(
          df = mapping_data,
          threshold_value = thresh$value,
          threshold_method = thresh$method,
          ci_size = ci_size,
          snp_grouping = snp_grouping,
          verbose = FALSE
        )
        ...
        result_row <- data.frame(
          ...
          maf = maf,
          status = "success",
          n_markers = summary_stats$n_markers,
          n_significant = summary_stats$n_significant,
          pct_significant = summary_stats$pct_significant,
          n_qtl = summary_stats$n_qtl,
          max_log10p = summary_stats$max_log10p,
          threshold_value = summary_stats$threshold_value,
          threshold_method = summary_stats$threshold_method,
          stringsAsFactors = FALSE
        )

        if (return_full_data) {
          attr(result_row, "full_data") <- list(
            processed = processed,
            qtl_regions = extract_qtl_regions(processed)
          )
        }
    ...
    # Process mappings (parallel or sequential)
    batch_indices <- batch_start:batch_end
    if (n_cores > 1 && length(batch_indices) > 1) {
      ...
      batch_results <- lapply(batch_indices, process_one)
    }
    ...
    for (j in seq_along(batch_indices)) {
      ...
      if (return_full_data && !is.null(attr(batch_results[[j]], "full_data"))) {
        full_data[[idx]] <- attr(batch_results[[j]], "full_data")
      }
    }
  }
  ...
  rate <- n_mappings / elapsed

  log_msg(glue::glue(
    "Batch complete: {n_success} success, {n_error} errors | ",
    "{round(elapsed, 1)}s total | {round(rate, 1)} mappings/sec"
  ))
