...
#   4. Mapping - association statistics (BETA, SE, P, log10p) per marker
#
# Storage organization:
#   - markers/{population}_{maf}_markers.parquet - static marker data per marker set
#   - mappings/{population}_mappings.parquet - mapping statistics per population
...
# in downstream Qmd scripts, not stored in the database.
...
#' close_cached_connection()  # Clean up when done
...
#'
#' @examples
#' result <- with_db_connection("data/db", {
#'   DBI::dbGetQuery(con, "SELECT COUNT(*) FROM mappings")
#' })
...
get_threshold_params <- function(population, maf, alpha = 0.05, base_dir = "data/db") {
  ...
  # read_marker_set_metadata returns a list or NULL
...
mappings_schema <- function() {
  arrow::schema(
    ...
    SE = arrow::float64(),
    P = arrow::float64(),
    log10p = arrow::float64(),
    var.exp = arrow::float64(),
    nqtl = arrow::int32(),
    rep = arrow::int32(),
    h2 = arrow::float64(),
    effect = arrow::utf8(),
...
#' Generate unique mapping ID from filename parameters
#'
#' @param params Named list from parse_mapping_filename()
#' @return Character string mapping ID
generate_mapping_id <- function(params) {
  pca_str <- if (params$pca) "_PCA" else ""
...
#' Also creates/updates marker set metadata. Only writes if marker set doesn't
#' already exist (markers are static).
#'
...
#'
#' Updates only the n_independent_tests and eigen_source_file fields
#' for an existing marker set metadata record.
#'
#' @param population Population identifier
#' @param maf MAF threshold
...
#'
#' Stores mapping-specific data (BETA, SE, P, log10p) for a single GWAS mapping.
...
prepare_mapping_data <- function(df, params) {
  ...
  mapping_cols <- c("marker", "N", "BETA", "SE", "P", "log10p", "var.exp", "trait")
...
prepare_mappings_for_parquet <- function(df) {
  ...
  num_cols <- c("BETA", "SE", "P", "log10p", "var.exp", "h2", "maf")
  for (col in num_cols) {
    if (col %in% names(df)) {
      df[[col]] <- as.numeric(df[[col]])
    }
...
#'
#' Processing steps:
#' 1. Generate unique mapping_id from params (nqtl, rep, h2, maf, effect, population, algorithm, pca)
#' 2. Prepare mapping data by selecting statistics columns and adding metadata columns
#' 3. Create partition directory structure for Hive-style partitioning
#' 4. Write Parquet file with snappy compression (from .db_constants)
#'
#' @param df Mapping dataframe (raw, deduplicated)
#' @param params Parsed filename parameters from parse_mapping_filename()
#' @param base_dir Database root directory
...
#' Compact partitioned mappings into single files per population
#'
#' Merges all partition files for a population into a single Parquet file.
#' Useful for reducing file count after batch imports.
#'
#' @param population Population to compact (NULL for all)
#' @param base_dir Database root directory
#' @return Invisibly returns compaction statistics
compact_partitions <- function(population = NULL, base_dir = "data/db") {
  config <- .make_db_config(base_dir)
  mappings_dir <- file.path(config$base_dir, config$mappings_dir)

  if (!dir.exists(mappings_dir)) {
    log_msg("No mappings directory found", level = "WARN")
    return(invisible(NULL))
  }

  # Find population directories
  pop_dirs <- list.dirs(mappings_dir, recursive = FALSE)
  pop_dirs <- pop_dirs[grepl("^population=", basename(pop_dirs))]

  if (!is.null(population)) {
    target_dir <- file.path(mappings_dir, glue::glue("population={population}"))
    if (!dir.exists(target_dir)) {
      stop(glue::glue("Population directory not found: {target_dir}"))
    }
    pop_dirs <- target_dir
  }
  ...
  for (pop_dir in pop_dirs) {
    pop_name <- sub("^population=", "", basename(pop_dir))
    log_msg(glue::glue("Compacting population: {pop_name}"))

    # Find all partition files
    partition_files <- list.files(pop_dir, pattern = "data\\.parquet$",
                                   recursive = TRUE, full.names = TRUE)

    if (length(partition_files) == 0) {
      log_msg(glue::glue("  No partition files found for {pop_name}"), level = "WARN")
      next
    }

    # Read and combine using DuckDB for efficiency
    con <- DBI::dbConnect(duckdb::duckdb(), ":memory:")
    on.exit(DBI::dbDisconnect(con, shutdown = TRUE), add = TRUE)

    file_pattern <- file.path(pop_dir, "**", "*.parquet")
    combined <- DBI::dbGetQuery(con, glue::glue("
      SELECT * FROM read_parquet('{file_pattern}', hive_partitioning = false, union_by_name = true)
    "))

    DBI::dbDisconnect(con, shutdown = TRUE)

    # Write compacted file
    compact_path <- file.path(mappings_dir, glue::glue("{pop_name}_mappings.parquet"))
    arrow::write_parquet(combined, compact_path, compression = config$compression)

    # Remove partition directories
    unlink(pop_dir, recursive = TRUE)
    ...
    log_msg(glue::glue("  Compacted {length(partition_files)} partitions -> {basename(compact_path)} ({nrow(combined)} rows)"))
  }
...
#' Compact database by rewriting Parquet files
#'
#' @param base_dir Database root directory
#' @return Invisibly returns statistics
compact_database <- function(base_dir = "data/db") {
  config <- .make_db_config(base_dir)
  markers_dir <- file.path(config$base_dir, config$markers_dir)
  mappings_dir <- file.path(config$base_dir, config$mappings_dir)
  ...
  all_files <- c(
    ...
    list.files(mappings_dir, pattern = "\\.parquet$", full.names = TRUE),
    file.path(config$base_dir, config$metadata_file)
  )

  all_files <- all_files[file.exists(all_files)]

  log_msg(glue::glue("Compacting {length(all_files)} files..."))

  for (pf in all_files) {
    old_size <- file.info(pf)$size

    data <- arrow::read_parquet(pf)
    arrow::write_parquet(
      data,
      pf,
      compression = config$compression,
    ...
    new_size <- file.info(pf)$size
    reduction <- round((1 - new_size / old_size) * 100, 1)
    log_msg(glue::glue("  {basename(pf)}: {reduction}% reduction"))
  }

  invisible(db_stats(base_dir))
}
