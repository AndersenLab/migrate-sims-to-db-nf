...
# Reading mapping files, loading configurations, EIGEN file handling
...
#' @param required_cols Columns that must be present (default from config)
...
read_mapping_file <- function(file_path,
                              required_cols = c("marker", "CHROM", "POS", "log10p"),
                              verbose = TRUE) {
  ...
  # Validate log10p values
  validate_log10p_values(df)
